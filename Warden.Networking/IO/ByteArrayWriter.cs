...
namespace Warden.Networking.IO
{
    class ByteArrayWriter : IByteWriter
    {
        ...
        {
            if (buffer.Length - (position + offset) < count)
            {
                //if (!bufferRented)
                    throw new IOException("EOF. Writer created with predefined buffer and can not be resized");

                //int newCapacity = buffer.Length * 2;
                //while (newCapacity < position + count)
                //    newCapacity *= 2;
        ...
        //public ArraySegment<byte> Allocate(int count)
        //{

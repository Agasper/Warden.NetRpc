...
namespace Warden.Networking.Tcp
{
    public class TcpConnectionStatistics
    {
        ...
        public float? AvgLatency => avgLatency;
        ...
        internal void UpdateLatency(float newLatency)
        {
            this.latency = newLatency;

            if (avgLatency.HasValue)
                avgLatency = (avgLatency.Value * 0.7f) + (newLatency * 0.3f);
            else
                avgLatency = newLatency;

            if (newLatency < TcpConnectionStatistics.UNSTABLE_CONNECTION_TIMEOUT)
                UnstableConnection = false;
        }
